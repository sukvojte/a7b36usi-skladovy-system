...
public class ProductService extends AbstractService implements IProductService {
    ...
    public Long saveProduct(ProductDTO product) {
        ...
        bo.setCategory(product.getCategory() != null ? genericDAO.loadById(
                product.getCategory(), CategoryBO.class) : null);
    ...
    public List<ProductDTO> getAllProducts() {
        ...
        for (ProductBO bo : bos) {
            ProductDTO dto = new ProductDTO(bo.getId(), bo.getName(),
                    bo.getCode(), bo.getQuantity(),
                    (bo.getCategory() != null ? bo.getCategory().getId() : 0));
            dtos.add(dto);
        }
        return dtos;
    }
    ...
    public Long saveCategory(CategoryDTO category) {
        ...
        bo.setParrent(category.getParrent() != null ? genericDAO.getById(
                category.getParrent(), CategoryBO.class) : null);
        return genericDAO.saveOrUpdate(bo).getId();
    }
    ...
    public List<CategoryDTO> getAllCategories() {
        ...
        for (CategoryBO bo : bos) {
            CategoryDTO dto = new CategoryDTO(bo.getId(), bo.getName(),
                    (bo.getParrent() != null ? bo.getParrent().getId() : null));
            dtos.add(dto);
        }
        return dtos;
    }
    ...
    public List<WrappingTypeDTO> getAllWrappingTypes() {
        ...
        for (WrappingTypeBO bo : bos) {
            WrappingTypeDTO dto = new WrappingTypeDTO(bo.getId(), bo.getName(),
                    bo.getQuantity(), bo.getProduct().getId());
            dtos.add(dto);
        }
        return dtos;
    }
    ...
    public List<ProductVersionDTO> getAllProductVersions() {
        ...
        for (ProductVersionBO bo : bos) {
            ProductVersionDTO dto = new ProductVersionDTO(bo.getId(),
                    bo.getCode(), bo.getName(), bo.getProduct().getId());
            dtos.add(dto);
        }
        return dtos;
    }
    ...
    public List<WrappingTypeDTO> getAllWrappingTypesByProduct(Long productId) {
        List<WrappingTypeDTO> dtos = new ArrayList<WrappingTypeDTO>();
        List<WrappingTypeBO> bos = genericDAO.getByProperty("product",
                genericDAO.loadById(productId, ProductBO.class), WrappingTypeBO.class);
        for (WrappingTypeBO bo : bos) {
            WrappingTypeDTO dto = new WrappingTypeDTO(bo.getId(), bo.getName(),
                    bo.getQuantity(), bo.getProduct().getId());
            dtos.add(dto);
    ...
    public List<ProductVersionDTO> getAllProductVersionsByProduct(Long productId) {
        List<ProductVersionDTO> dtos = new ArrayList<ProductVersionDTO>();
        List<ProductVersionBO> bos = genericDAO.getByProperty("product",
                genericDAO.loadById(productId, ProductBO.class), ProductVersionBO.class);
        for (ProductVersionBO bo : bos) {
            ProductVersionDTO dto = new ProductVersionDTO(bo.getId(),
                    bo.getCode(), bo.getName(), bo.getProduct().getId());
            dtos.add(dto);
