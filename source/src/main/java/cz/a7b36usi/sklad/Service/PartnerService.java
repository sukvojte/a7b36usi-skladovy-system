...
class PartnerService extends AbstractService implements IPartnerService {
	...
	public List<PartnerDTO> getAllPartners() {
		...
		for (PartnerBO zakaznik : bolist) {
			partners.add(new PartnerDTO(zakaznik.getId(), zakaznik
					.getIsDodavatel(), zakaznik.getIsOdberatel(), zakaznik
					.getUlice(), zakaznik.getMesto(), zakaznik.getSpolecnost(),
					zakaznik.getPsc(), zakaznik.getCisloPopisne()));
		}
		return partners;
	}
