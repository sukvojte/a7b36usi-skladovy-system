...
public class DocumentService extends AbstractService implements
	...
	public List<DocumentDTO> getAllDocuments() {
		...
		for (DocumentBO documentBO : bos) {
			...
			DocumentDTO dto = new DocumentDTO(documentBO.getId(),
					documentBO.getDocumentType(), partner,
					documentBO.getNumber(), documentBO.getDate());
			dtos.add(dto);
		}
		return dtos;
	}
	...
	public List<MovementDTO> getAllMovements() {
		...
		for (MovementBO movementBO : bos) {
			...
			ProductDTO product = new ProductDTO(pbo.getId(), pbo.getName(),
					pbo.getCode(), pbo.getQuantity(),
					(pbo.getCategory() != null ? pbo.getCategory().getId() : 0));

			MovementDTO dto = new MovementDTO(movementBO.getId(),
					movementBO.getPrice(),
					(movementBO.getWrapping() != null ? movementBO
							.getWrapping().getId() : 0),
					(movementBO.getVersion() != null ? movementBO.getVersion()
							.getId() : 0), product, getDocumentById(movementBO
							.getDocument().getId()), movementBO.getQuantity());
	...
	public List<MovementDTO> getAllDocumentsMovements(Long documentId) {
		List<MovementDTO> dtos = new ArrayList<MovementDTO>();
		List<MovementBO> bos = genericDAO.getByProperty("document",
				genericDAO.loadById(documentId, DocumentBO.class),
				MovementBO.class);
		for (MovementBO movementBO : bos) {
			ProductBO pbo = movementBO.getProdukt();
			ProductDTO product = new ProductDTO(pbo.getId(), pbo.getName(),
	...
	public List<DocumentDTO> getAllPartnersDocuments(Long partnerId) {
		List<DocumentDTO> dtos = new ArrayList<DocumentDTO>();
		List<DocumentBO> bos = genericDAO.getByProperty("partner",
				genericDAO.loadById(partnerId, PartnerBO.class),
				DocumentBO.class);
		for (DocumentBO documentBO : bos) {
	...
	public List<MovementDTO> getAllProductsMovements(Long productId) {
		List<MovementDTO> dtos = new ArrayList<MovementDTO>();
		List<MovementBO> bos = genericDAO.getByProperty("product",
				genericDAO.loadById(productId, ProductBO.class),
				MovementBO.class);
		for (MovementBO movementBO : bos) {
			ProductBO pbo = movementBO.getProdukt();
			ProductDTO product = new ProductDTO(pbo.getId(), pbo.getName(),
	...
	public DocumentDTO getDocumentById(Long documentId) {
		...
		PartnerDTO partner = new PartnerDTO(pbo.getId(), pbo.getIsDodavatel(),
				pbo.getIsOdberatel(), pbo.getUlice(), pbo.getMesto(),
				pbo.getSpolecnost(), pbo.getPsc(), pbo.getCisloPopisne());

		return new DocumentDTO(document.getId(), document.getDocumentType(),
				partner, document.getNumber(), document.getDate());
	}
